...
### D19 — Customary units are a subpackage: `units/imperial`

A sister package mapping customary units — foot, stone, psi, BTU, gallon — is
planned, and it goes in this module, under `units/`, generated by the same
`catgen` run from a catalogue file of its own.
...
**The one exception it forces.** `units/imperial` holds more than one dimension,
...
The alternative was `units/imperial/length` beside `units/length`, and it is
...
- **The gallon, the fluid ounce and the ton**, because the US and the imperial
  ...
  and it is O3 in section 10: `units/imperial` cannot correctly hold a US gallon.
  A second package does not help — the symbol index of D12 is global — so it is a
  spelling question before it is a package question. A unit whose package name
  contradicts its definition is worse than a missing unit.
...
`units/imperial` (D19) holds the customary units, which are several dimensions
...
- ~~`O1`, whether `imperial` is a subpackage or a module~~ — settled as D19: a
  subpackage, `units/imperial`, from a catalogue file of its own
- `O3` in section 10, which names the package the two customary systems share.
  It is a `v1.0.0` item for one reason only: it is an import path, and D18 is the
  precedent for how cheap that is to change before a release and how expensive
  after
...
Two are settled, and building the second turned up a third:

| | |
|---|---|
| O1 — non-SI units: subpackage or separate module? | **D19** — a subpackage, `units/imperial`, generated by the same run from a catalogue of its own |
...
| O3 — what covers the units the two customary systems disagree about? | **open**, below; a `v1.0.0` question because it names an import path |

The settled entries stay rather than being deleted: a reader who remembers the
question should find the answer where the question was.

### O3 — `units/imperial` cannot hold a US gallon

**Status:** open, with a recommendation; decide before `v1.0.0`

D19 shipped the eight customary units that are the *same* in both systems — the
inch, foot, yard and mile, the pound and ounce, the pound-force, psi — and left
out the ones that are not: the gallon, the fluid ounce, the ton. The US liquid
gallon is 231 in³ and the imperial gallon is 4.54609 L, a fifth apart, and a
package called `imperial` holding the first would be a name contradicting a
definition. That is worse than a missing unit, so the units are missing.

**Two packages do not solve it, and knowing why is most of the answer.** The
symbol index of D12 is global: `catgen` keys it by spelling and kind with no
package in the key, because `catalog.BySymbol` and `parse` resolve a symbol
against the whole catalogue and a spelling that resolves to two units is a text
form that does not read back. So `units/uscustomary.Gallon` and
`units/imperial.Gallon` both spelling `gal` is rejected by the generator, in
exactly the way it is meant to be. **The question is a spelling question first
and a package question second.**

**Recommendation: one package, renamed `units/customary`, and two spellings that
are not both `gal`.** The eight units already shipped are common to both systems,
so the rename costs nothing but an import path — which is precisely why it has to
happen before `v1.0.0` rather than after. What the spellings should be is the
part that needs a decision rather than a preference: NIST SP 811 writes "gallon
(U.S.)" and "gallon [Canadian and U.K. (Imperial)]" in words, so the standard
offers no distinct symbol and anything short is this library's invention. An
invented notation is a real cost — D12 spends most of its length on not inventing
one — and it has to be weighed against simply shipping one of the two gallons and
saying which.

The British thermal unit is the same question in a second form and should get the
same answer: at least five units — International Table, thermochemical, and three
more — share the spelling `Btu`, and D12 cannot hold two units under one symbol.
